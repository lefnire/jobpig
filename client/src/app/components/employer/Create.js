...
import {_fetch, getTags, constants, filterOptions, _ga, setAnon} from '../../helpers';
...
export default class CreateJob extends React.Component {
  ...
  render() {
    ...
    let content = (
      <Modal.Body>
        ...
        <Formsy.Form
          ...
          onValidSubmit={demo ? this.submitDemo : this.submitForm}
        >
          <FormsyText
            name='title'
            required
    ...
    let title =
      free_jobs ? `${free_jobs} Free Post${free_jobs>1? 's': ''}`
  ...
  submitDemo = body => {
    ...
    body.location = location && location.value;
    body.tags = tags && _.map(tags, 'value');
    _fetch('jobs', {method: "POST", body}).then(this.props.onSubmit);
  };
